...
def train(image_size: int, wandb_logging: bool, train_batch_size: Optional[int], val_batch_size: Optional[int]):
    ...
    processor = PatchPackProcessor(config, size={"height": config.image_size, "width": config.image_size})
    model = PatchPackModelImageClassification(config)
    model.gradient_checkpointing_enable()

    collate_fn = partial(collate, processor)

    # TODO: update the batch sizes here
    ...
    for epoch in range(BASELINE_CONFIG['max_epochs']):
        for i, batch in enumerate(tqdm(train_dataloader)):
            ...
            with autocast(dtype=torch.float16):
                ...
                x = resize_with_aspect_ratio(images.pixel_values, image_size)
                outputs = model(pixel_values=x.to(device), labels=labels.to(device))
                loss = outputs.loss
                ...
                if i > 0 and i % BASELINE_CONFIG['val_steps'] == 0:
                    with torch.no_grad():
                        ...
                        for val_batch in tqdm(val_dataloader, position=1):
                            ...
                            x = resize_with_aspect_ratio(images.pixel_values, image_size)
                            out = model(pixel_values=x.to(device), labels=labels.to(device))
